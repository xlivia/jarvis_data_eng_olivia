    ...
    public void run(String[] args) {
        if (args.length == 0) {
            ...
            switch (args[0].toLowerCase()) {
                case "post" -> this.printTweet(this.controller.postTweet(args));
                case "show" -> this.printTweet(this.controller.showTweet(args));
                case "delete" -> this.controller.deleteTweet(args).forEach(this::printTweet);
                default -> throw new IllegalArgumentException("USAGE: TwitterCLIApp post|show|delete [options]");
            }
